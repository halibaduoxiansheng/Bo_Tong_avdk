...
//	aud_intf_info.api_info.busy_status = true;
	...
	if (work_mode == AUD_INTF_WORK_MODE_GENERAL || work_mode == AUD_INTF_WORK_MODE_VOICE) {
		...
		if (work_mode == AUD_INTF_WORK_MODE_VOICE) {
			...
			// send_audio_data_to_agora  NOTE add i4s aud func 
			/* NOTE 调用位置 aud_tras.c 中 */
			aud_tras_setup_cfg.i4s_aud_tras_send_data_cb = aud_intf_info.drv_info.setup.i4s_aud_intf_tx_mic_data;
			aud_tras_setup_cfg.aud_tras_send_data_cb = aud_intf_info.drv_info.setup.aud_intf_tx_mic_data;
			ret = aud_tras_init(&aud_tras_setup_cfg); // NOTE 关键函数
			if (ret != BK_OK) {
...
bk_err_t bk_aud_intf_voc_init(aud_intf_voc_setup_t setup)
{
	...
	aud_intf_info.voc_info.mic_en = setup.mic_en; /* NOTE 均启动 */
	aud_intf_info.voc_info.spk_en = setup.spk_en;
	aud_intf_info.voc_info.mic_type = setup.mic_type; /* NOTE 均板载 */
	aud_intf_info.voc_info.spk_type = setup.spk_type;

	/* aec config */
	if (aud_intf_info.voc_info.aec_enable) { /* NOTE 如果启动了 AEC 就配置 AEC的进一步参数 */
...
#if CONFIG_AUD_INTF_SUPPORT_G722
		case AUD_INTF_VOC_DATA_TYPE_G722:
			aud_intf_info.voc_info.rx_info.frame_size = 160;		//apk receive one frame 40ms
			//aud_intf_info.voc_info.rx_info.frame_size = aud_intf_info.voc_info.aud_setup.mic_samp_rate_points;
			break;
#endif
	...
	// NOTE 如果认为本地处理的太慢，给的东西存不进来 可以增大这个值 并通知增大 speaker_frame_number = 2 位置
	aud_intf_info.voc_info.rx_info.frame_num = setup.frame_num; // 15 走的 DEFAULT_AUD_INTF_VOC_SETUP_CONFIG
	aud_intf_info.voc_info.rx_info.rx_buff_seq_tail = 0;
	aud_intf_info.voc_info.rx_info.fifo_frame_num = setup.fifo_frame_num; /* NOTE 10 */
	...
	/* NOTE mic 原厂 mic 数据 发送位置 */
	aud_intf_info.voc_info.aud_tx_rb = aud_tras_get_tx_rb();

	aud_intf_info.voc_info.i4s_aud_tx_rb = i4s_aud_tras_get_tx_rb();

	// i4s_aud_tras_set_rx_rb(&aud_intf_info.voc_info.i4s_aud_rx_rb);
	...
	switch (aud_intf_info.voc_status) {
		...
		case AUD_INTF_VOC_STA_STOP:
			...
			if (ring_buffer_get_fill_size(aud_intf_info.voc_info.rx_info.decoder_rb)/aud_intf_info.voc_info.rx_info.frame_size < aud_intf_info.voc_info.rx_info.fifo_frame_num) {
				/* NOTE 计算 decoder_rb 中存储的 数据帧 < FIFO 最小播放帧数 */			
				...
				if (temp_buff == NULL) {
					...
					switch (aud_intf_info.voc_info.data_type) { /* NOTE 设置静止音 */
						case AUD_INTF_VOC_DATA_TYPE_G711A:
...
bk_err_t bk_aud_intf_write_spk_data(uint8_t *dac_buff, uint32_t size)
{
	...
	switch (aud_intf_info.drv_info.setup.work_mode) {
		...
		case AUD_INTF_WORK_MODE_VOICE: /* AI 对话中 走这条分支 也即是数据需要解压 */
#ifdef CONFIG_AUD_RX_COUNT_DEBUG
...
{
	if (aud_intf_info.voc_status == AUD_INTF_VOC_STA_NULL) {
		bk_printf("local status is null\r\n");
		return BK_ERR_AUD_INTF_STA;
	}
	// bk_printf("send volume msg success\r\n");
