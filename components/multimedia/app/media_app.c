...
static beken_queue_t media_app_msg_queue = NULL; /* 服务于 UVC 的 */
...
/* NOTE agora_rtc_demo.c -> video_turn_on -> media_checkout_uvc_device_info */
...
bk_err_t media_app_lvgl_open(void *lcd_open)
{
	...
	// 这里的 EVENT_LVGL_OPEN_IND 会进一步调用 LVGL_EVENT
...
static void media_app_message_handle(void)
{
	...
	while (1)
	{
		...
		if (BK_OK == ret)
		{
			switch (msg.event)
			{
				case EVENT_UVC_DEVICE_INFO_NOTIFY: /* UVC 模块 */
					if (uvc_device_info_cb) /* media_app_uvc_register_info_notify_cb 注册 */
					{
						if (msg.param != 0)
    ...
    if (media_app_asr_evt_cb)
    {
        switch (msg->event)
        {
            ...
            case EVENT_ASR_RECEIVE_ADDRESS:
            	media_app_asr_evt_cb(MEDIA_APP_EVT_RECEIVE_ADDRESS, msg->param);
                break;
#if I4S_RECORD_REPEAT_MODE
            case EVENT_ASR_OPEN_RECORD_IND:
            	media_app_asr_evt_cb(MEDIA_APP_EVT_ASR_OPEN_RECORD_IND, msg->param);
            	break;
            case EVENT_ASR_CLOSE_RECORD_IND:
            	media_app_asr_evt_cb(MEDIA_APP_EVT_ASR_CLOSE_RECORD_IND, msg->param);
            	break;
#endif
        }
...
bk_err_t media_app_init(void)
{
	...
	ret = rtos_create_thread(&media_app_th_hd,
	                         ...
	                         "media_app_thread",
	                         (beken_thread_function_t)media_app_message_handle, /* UVC */
	                         2048,
	                         NULL);
