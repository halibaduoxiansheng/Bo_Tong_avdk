...
typedef struct
{
	...
	uint8_t *i4s_aud_tras_buff_addr;
	RingBufferContext i4s_aud_tras_rb;  /* NOTE i4s aud use */
	...
	while (1) {
		...
		if (kNoErr == ret) {
			switch (msg.op) {
				...
				case AUD_TRAS_TX:
					/* NOTE I4S */
					fill_size = ring_buffer_get_fill_size(&aud_tras_info->i4s_aud_tras_rb);
					for (int n = 0; n < fill_size/320; n++) {
#if (CONFIG_CACHE_ENABLE)
							flush_all_dcache();
#endif
							GLOBAL_INT_DISABLE();
							ring_buffer_read(&aud_tras_info->i4s_aud_tras_rb, aud_temp_data, 320);
							GLOBAL_INT_RESTORE();
							// NOTE send_audio_data_to_agora -> aud_intf.c 中被赋值地址 x -> agora_rtc_demo.c 注册地址x指向函数
							if (aud_trs_setup->i4s_aud_tras_send_data_cb) /*  */
								aud_trs_setup->i4s_aud_tras_send_data_cb(aud_temp_data, 320);
					}


					/* NOTE 下面是与声网交互 这里的数据格式是 G722 上面的才是 PCM */
...
#endif					
						// NOTE send_audio_data_to_agora -> aud_intf.c -> agora_rtc_demo
...
//						GPIO_DOWN(6);
					...
					// NOTE agora_aud_tras_main 中也是 5 秒一次
...
aud_tras_exit:
	...
	if (aud_tras_info->i4s_aud_tras_buff_addr) {
		ring_buffer_clear(&aud_tras_info->i4s_aud_tras_rb);
		audio_tras_free(aud_tras_info->i4s_aud_tras_buff_addr);
		aud_tras_info->i4s_aud_tras_buff_addr = NULL;
	}
...
bk_err_t aud_tras_init(aud_tras_setup_t *setup_cfg)
{
	...
	/* NOTE 下面是 i4s aud use */
	aud_tras_info->i4s_aud_tras_buff_addr = audio_tras_malloc(AUD_TRAS_BUFF_SIZE + CONFIG_AUD_RING_BUFF_SAFE_INTERVAL);
	if (!aud_tras_info->i4s_aud_tras_buff_addr) {
		LOGE("malloc i4s_aud_tras_buff_addr\n");
		goto out;
	}
	ring_buffer_init(&aud_tras_info->i4s_aud_tras_rb, aud_tras_info->i4s_aud_tras_buff_addr, AUD_TRAS_BUFF_SIZE + CONFIG_AUD_RING_BUFF_SAFE_INTERVAL, DMA_ID_MAX, RB_DMA_TYPE_NULL);	
	...
	if ((!aud_tras_info->aud_tras_task_hdl) && (!aud_tras_info->aud_tras_int_msg_que))
	{
		...
		ret = rtos_create_thread(&aud_tras_info->aud_tras_task_hdl,
								 4,
								 "aud_tras",
								 (beken_thread_function_t)aud_tras_main, // NOTE
								 1024 * 2,
...
RingBufferContext *i4s_aud_tras_get_tx_rb(void)
{
	return &aud_tras_info->i4s_aud_tras_rb;
}
