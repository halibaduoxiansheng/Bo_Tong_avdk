 ...
 *    - BK_OK: succeed
 *    - others: other errors.
 *    调节 传入 这个 g_volume_gain[SPK_VOLUME_LEVEL];  就好了
 */
 ...
 *    - others: other errors.
 */
/* TODO 是不是应该将 vad 放在这个开启的里面然后去开始检测，没开启肯定是不需要检测的 */
bk_err_t bk_aud_intf_mic_start(void);    /* TODO 越来越看不懂了 为什么都没有调用这种函数还能收集音频的？ */
