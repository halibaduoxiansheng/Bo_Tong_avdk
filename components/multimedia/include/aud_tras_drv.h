...
/* I4S Configure Start */
/* 目前不知道放在哪个位置，所以有两个一样的宏定义名称 另在 agora_config.h 中 */
#define I4S_USE_VAD_DEPLOY  (1) /* 开启就是使用 VAD ，检测到有人说话才会传输声音 */
#define I4S_USE_VAD_LIB		(1) /* 1: 使用自己的 VAD  */



#define I4S_RECORD_REPEAT_MODE 	(1)  /* 是否开启 录音重复模式 */
/* I4S Configure End */
...
typedef enum {
	...
	AUD_TRAS_DRV_MIC_DEINIT,	/**< 5 deinit mic */
	AUD_TRAS_DRV_MIC_START,		/**< start mic */
	AUD_TRAS_DRV_MIC_PAUSE,		/**< pause mic */
	AUD_TRAS_DRV_MIC_STOP,		/**< stop mic  stop会清空Mic缓存 */
	AUD_TRAS_DRV_MIC_SET_CHL,	/**< set mic channel */
	AUD_TRAS_DRV_MIC_SET_SAMP_RATE,		/**< 10 set adc sample rate */
	...
	AUD_TRAS_DRV_SPK_START,		/**< 15 start spk */
	...
	AUD_TRAS_DRV_SPK_SET_GAIN,			/**< 20 set dac gain */
	/* spk int op */
	...
	AUD_TRAS_DRV_VOC_STOP,		/**< 25 stop voc */
	...
	AUD_TRAS_DRV_VOC_SET_SPK_GAIN,		/**< 30 set audio dac gain */
	...
	AUD_TRAS_DRV_VOC_AEC_DEBUG,			/**< 35 dump aec data */
	/* voc int op */
...
//	AUD_TRAS_DRV_TX_DONE,
	...
	AUD_TRAS_DRV_UAC_AUTO_CONT_CTRL,			/**< 45 uac automatically connect enable control*/
    ...
    AUD_TRAS_OPEN_RECORD_MODE,
    AUD_TRAS_CLOSE_RECORD_MODE,
#endif
	AUD_TRAS_DRV_MAX,
} aud_tras_drv_op_t;
...
typedef struct {
	...
	int16_t* ref_addr; /* NOTE ref_addr 与 mic_addr 配合使用，mic_addr 为近端麦克风，ref_addr 为麦克风可能采集到的远端喇叭声音 */
	int16_t* mic_addr;

	int16_t* out_addr; /* AEC 处理后（结合 ref_addr 和 mic_addr） 地址 */

	/* NOTE (本身未使用) 提供两个接口，已经malloc 使用直接使用就好了 */
	int16_t *aec_ref_ring_buff; //save ref data of aec
	int16_t *aec_out_ring_buff; //save audio data processed by AEC

	RingBufferContext ref_rb;   //ref data of AEC context   ( 接收到数据 解码后就是这个里面 )
	RingBufferContext aec_rb;   //out data of AEC context   ( mic 数据 ，如果开启了 AEC 功能， aec效果后 储存在这里面 )
} aec_info_t;
...
typedef struct {
	int16_t *pcm_data;
	uint8_t *law_data; /* NOTE 指向解码后的数据 */
...
typedef struct {
	...
	int32_t *mic_ring_buff;				//save mic data from audio adc (dma 直接搬数据使用的buf 似乎并没有使用 mic_rb )

	RingBufferContext mic_rb;			//mic ring buffer context （存储的mic采集的原始数据）
	...
	int32_t *speaker_ring_buff;			//save dac data of speaker （dma 直接搬数据使用的buf）
	RingBufferContext speaker_rb;		//speaker ring buffer context   ( NOTE 从远端收到，(解压后)由dma搬运，如果移除将导致出声嘶哑)
...
#endif
#endif
	RingBufferContext *aud_tx_rb;  /* NOTE 这个里面存储的是发送给远端服务器的内容 */
	RingBufferContext *i4s_aud_tx_rb; /* I4S use */
...
extern aud_tras_drv_info_t aud_tras_drv_info;
...
#if CONFIG_AUD_INTF_SUPPORT_MULTIPLE_SPK_SOURCE_TYPE
typedef enum {
	SPK_SOURCE_TYPE_VOICE = 0, // 对话
	SPK_SOURCE_TYPE_PROMPT_TONE, // 系统提示音
	SPK_SOURCE_TYPE_A2DP, // A2DP 是 bluetooth 的高级音频分发协议，代表播放手机音乐
	SPK_SOURCE_TYPE_MAX, // 
} spk_source_type_t;
