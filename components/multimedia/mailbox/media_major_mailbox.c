...
static LIST_HEADER_T media_major_mailbox_msg_queue_req; /* major、minor、APP of module */
...
static beken_semaphore_t media_major_mailbox_sem = NULL; /* media_major_mailbox_msg_queue_req 使用 */
static beken_semaphore_t media_major_mailbox_rsp_sem = NULL;
static beken_semaphore_t media_major_mailbox_init_sem = NULL;

static uint8_t media_major_mailbox_inited = 0; /* media_major_mailbox_init 函数是否已初始化 */

static bk_err_t media_send_msg_to_queue(media_mailbox_msg_t *param, uint8_t ori);

/* NOTE msg.type = MAILBOX_MSG_TYPE_REQ; */
...
/* REVIEW 顾名思义 sync 是与另外两个mailbox同步，所以dest不是major本身的 */
...
static bk_err_t media_major_mailbox_send_msg_to_media_minor_mailbox(media_mailbox_msg_t *msg)
{
	...
	{
		ret = rtos_get_semaphore(&media_major_mailbox_rsp_sem, 2000); /* waiting for send complete */
		if(ret != BK_OK)
		{
			LOGE("%s %d rtos_get_semaphore error type=%x event=%x\n", __func__, __LINE__, msg->type, msg->event);
			if (msg->type == MAILBOX_MSG_TYPE_REQ)
...
static void media_major_mailbox_mailbox_rx_isr(void *param, mb_chnl_cmd_t *cmd_buf)
{
	...
	if (msg)
	{
		...
		media_send_msg_to_queue(msg, MEDIA_EVENT_REQ); // NOTE （启动） dest to major 的来源
	}
	else
	{
...
{
	media_major_mailbox_mailbox_rx_isr(param, cmd_buf); /* 同下 */
}

static void media_major_mailbox_mailbox_from_cp2_rx_isr(void *param, mb_chnl_cmd_t *cmd_buf)
{
	media_major_mailbox_mailbox_rx_isr(param, cmd_buf); /* 同上 */
}
	...
	if (msg->src == APP_MODULE)
	{
		if(msg->type == MAILBOX_MSG_TYPE_REQ) //send req msg to cpu1 other threads
		{
			switch (msg->event >> MEDIA_EVT_BIT)
			{
				...
				case LVGL_EVENT: // NOTE 
					media_msg.event = msg->event; /* ==> EVENT_LVGL_OPEN_IND */
					media_msg.param = (uint32_t)msg; /* 这里是 lcd_open_t */
					media_send_msg(&media_msg); /* 前往文件 media_ui.c media_major 和 media_minor 没有参与编译 */
					break;
				...
				/* NOTE 位移运算符  */
				case AUD_EVENT: // EVENT_AUD_INIT_REQ = (AUD_EVENT << MEDIA_EVT_BIT) This  Function ( bk_aud_intf_drv_init )
				case AUD_NOTIFY: /* NOTE 通过上面的触发 没有通过这个触发过 */
					{
					// os_printf("this this this\r\n"); // 有
#if (CONFIG_AI_AGENT)
					extern bk_err_t audio_control_event_handle(media_mailbox_msg_t * msg);
                    audio_control_event_handle(msg);
#else
                    // NOTE THIS （有打印） adc 或 dac  来源 -> bk_aud_intf_drv_init -> agora_rtc_demo.c 中的 audio_turn_on
	...
	else if (msg->src == MINOR_MODULE)
	{
		...
		else if(msg->type == MAILBOX_MSG_TYPE_NOTIFY)
		{
			switch (msg->event)
			{
				...
				case EVENT_AUD_CP2_READY_NOTIFY:
				{
					aud_cp2_ready_notify();
				}
				break;

				case EVENT_ASR_RESULT_NOTIFY:
				{
					/* NOTE ASR 结果是这里出去的 */
					aud_tras_drv_set_dialog_run_state_by_asr_result(msg->result);
				}
				break;
#endif
				default:
...
static void media_major_mailbox_message_handle(void)
{
	...
	while (1)
	{
		...
		if (node_msg != NULL)
		{
			...
			if (node_msg->count > 2) /* NOTE mailbox 失败两次以上未发送成功 */
			{
			...
			{
				case MAJOR_MODULE: /* NOTE 队列信息来源 media_send_msg_to_queue （event == MEDIA_EVENT_REQ） 也就是需要 别处通知 */
		...
		/* 唯一入队列位置 media_send_msg_to_queue */
		while(!rtos_is_queue_empty(&media_major_mailbox_msg_queue)) // 队列非空再去取
		{
			/* 唯一向队列存入位置  media_send_msg_to_queue  */
...
bk_err_t media_major_mailbox_init(void)
{
	...
	if (media_major_mailbox_inited != 0) /* NOTE 线程函数是否创建 */
	{
		LOGE("media_major_mailbox already init, exit!\n");
		return ret;
	}

	ret = rtos_init_semaphore(&media_major_mailbox_sem, 1); /* NOTE ensure mailbox rx safe */
	if (ret != BK_OK)
	{
	...
	ret = rtos_init_semaphore(&media_major_mailbox_rsp_sem, 1); /* NOTE ensure mailbox tx complete */
	if (ret != BK_OK)
	{
	...
	ret = rtos_init_semaphore(&media_major_mailbox_init_sem, 1); /* NOTE ensure thread safe semaphore */
	if (ret != BK_OK)
	{
