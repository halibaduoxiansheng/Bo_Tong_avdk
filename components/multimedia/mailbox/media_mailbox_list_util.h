...
typedef struct
{
    ...
    uint32_t result;
    uint32_t type;
    uint32_t param; /* lcd_open_t * */
    uint8_t src;
    uint8_t dest;
    uint8_t count; /* 链表中 MAILBOX_MSG_TYPE_REQ 类型数据 个体 通知次数 （==> 通知该进行但没做的次数）*/
    uint8_t ack_flag; /* MAILBOX_MESSAGE_ACK after MAILBOX_MESSAGE_SEND */
} media_mailbox_msg_t;
...
enum
{
    MAILBOX_MSG_TYPE_REQ = 0, // request
    MAILBOX_MSG_TYPE_RSP,     // response
    MAILBOX_MSG_TYPE_NOTIFY,  // notify
    MAILBOX_MSG_TYPE_ABORT,   // abort
} ;
