...
static beken_semaphore_t media_minor_mailbox_init_sem = NULL; /*确保线程完全准备好*/

static uint8_t media_minor_mailbox_inited = 0; /* media_minor_mailbox_message_handle 函数是否已初始化 */

static bk_err_t media_send_msg_to_queue(media_mailbox_msg_t *param, uint8_t ori);

/* 先将内容放入 链表中 然后通过信号量控制 从链表中拿出 再使用 mailbox 去发送 */
...
static void media_minor_mailbox_mailbox_rx_isr(void *param, mb_chnl_cmd_t *cmd_buf)
{
	...
	if (msg)
	{
		...
		media_send_msg_to_queue(msg, MEDIA_EVENT_REQ);
	}
	else
	{
...
void media_minor_mailbox_msg_handle(media_mailbox_msg_t *msg)
{
	...
	if (msg->src == MAJOR_MODULE)
	{
		if(msg->type == MAILBOX_MSG_TYPE_REQ) //send req msg to cpu2 other threads
		{
			switch (msg->event)
			{
				...
				{
					// XXX 语音识别开启
					extern bk_err_t armino_asr_open(void);
					ret = armino_asr_open();
				}
				break;

				case EVENT_ASR_DEINIT_REQ:
				{
					extern bk_err_t armino_asr_close(void);
					ret = armino_asr_close();
				}
				break;
#endif
		...
		else if(msg->type == MAILBOX_MSG_TYPE_NOTIFY)
		{
			switch (msg->event)
			{
				...
				{
					/* NOTE 这个地方 讲录制到的音频AEC后回调给	ASR 识别 
						也许是 bk_aud_debug_voc_aec_tcp_send_packet 函数 要么就是NULL */
					extern int aec_output_callback(void *asr_data, void *user_data);
					aec_output_callback((void *)msg->param, NULL);
				}
				break;
#endif
	...
	{
		bk_printf("this msg is not from MAJOR_MODULE.is from %d\r\n", msg->src);
	}
}
...
static void media_minor_mailbox_message_handle(void)
{
	...
	/* NOTE Major 执行 media_major_mailbox_send_msg_to_media_minor_mailbox */
	msg_send_notify_to_media_minor_mailbox(&jpeg_dec_to_media_major_msg, MAJOR_MODULE);

#if CONFIG_AI_ASR_MODE_CPU2
	media_mailbox_msg_t asr_to_media_major_msg = {0};
	asr_to_media_major_msg.event = EVENT_AUD_CP2_READY_NOTIFY;
	...
	while (1)
	{
		...
		if (node_msg != NULL)
		{
			...
			if (node_msg->count > 2) /* 讯息 大于两次未能成功发送出去 */
			{
		...
		else // media_minor_mailbox_msg_queue_req 链表中出不了 才获取信号量
		{
...
/* NOTE 
 * app、minor、major 三大mailbox 结构相似
 * 信号量控制 rx、tx，thread safe，并且使用 req 和 rsp 两大链表
 * req 链表 存自己需要向其他两大mailbox的讯息
 * rsp 链表 存自己mailbox讯息为 MAILBOX_MSG_TYPE_REQ 的 
 * 且三大mailbox都有一个 队列 ，其中 rsp 链表存入 完全依赖 队列
 *  而 req 多一个 sync 函数 加入方式
*/
bk_err_t media_minor_mailbox_init(void)
{
	...
	ret = rtos_init_semaphore(&media_minor_mailbox_sem, 1); /* NOTE ensure mailbox rx safe */
	if (ret != BK_OK)
	{
	...
	ret = rtos_init_semaphore(&media_minor_mailbox_rsp_sem, 1); /* NOTE ensure mailbox tx complete */
	if (ret != BK_OK)
	{
	...
	/* 确保线程正常执行 信号量 */
	ret = rtos_init_semaphore(&media_minor_mailbox_init_sem, 1); /* NOTE ensure thread safe semaphore */
	if (ret != BK_OK)
	{
