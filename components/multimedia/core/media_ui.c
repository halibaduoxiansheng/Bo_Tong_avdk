...
#endif
#ifdef CONFIG_LVGL
				case LVGL_EVENT: // XXX 唯一调用
					mb_msg = (media_mailbox_msg_t *)msg.param;
					bk_printf("come here 1\r\n");
...
bk_err_t media_ui_task_init(void)
{
	...
	if ((!media_ui_task) && (!media_ui_msg_que)) /*防止重复进入*/
	{
...
#ifdef CONFIG_IMAGE_STORAGE /*图片存储功能*/
	storage_init();
#endif
