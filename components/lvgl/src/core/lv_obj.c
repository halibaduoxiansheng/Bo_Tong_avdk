...
void lv_init(void) 
{
    ...
    _lv_timer_core_init(); /*负责控件动画，刷新定时任务*/

    _lv_fs_init(); /*负责 文件读写*/

    _lv_anim_core_init(); /* 负责 比如 按钮变色，滑动动画 */

    _lv_group_init(); /* 负责 用于多个对象的焦点管理，尤其是键盘、编码器控制界面时用到 */

    lv_draw_init(); /* 负责 实际把LVGL控件动画显示到显存或暂存区，最后刷到屏幕 */
