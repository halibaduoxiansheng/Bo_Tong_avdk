...
int lv_port_disp_init(void)
{
    ...
    lv_disp_drv_init(&disp_drv); /*Basic initialization  初始化 显示驱动结构体 */
    ...
    disp_drv.flush_cb = disp_flush; // XXX
    ...
    lv_disp_drv_register(&disp_drv); // 显示设备对象的句柄 （注册显示驱动）
