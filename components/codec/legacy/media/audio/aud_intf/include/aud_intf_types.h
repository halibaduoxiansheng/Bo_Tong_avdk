...
typedef enum {
	...
	AUD_INTF_WORK_MODE_GENERAL, /** 非交互性的，播放或录制 < General mode: support record mic data and play speaker */
	AUD_INTF_WORK_MODE_VOICE, /** 实时语音通话处理 < Voice mode: support voice transfer 传给 ASR */
...
typedef struct {
	aud_intf_work_mode_t work_mode; /* aud_intf_work_mode_t */
	aud_intf_task_config_t task_config; /* 暂且只有优先级 */
	/**< the api is called when collecting a frame mic packet data is complete */
	int (*aud_intf_tx_mic_data)(unsigned char *data, unsigned int size);		
	/**< the api is called when playing a frame speaker packet data is complete */
	bk_err_t (*aud_intf_rx_spk_data)(unsigned int size);						
...
typedef enum {
	AUD_INTF_MIC_TYPE_BOARD = 0, /* 板载或本地麦克风 */
	AUD_INTF_MIC_TYPE_UAC, /* USB麦克风 */
...
typedef enum {
	...
	AUD_INTF_VOC_DATA_TYPE_PCM, /**< the data of voice transfer is PCM 未压缩 */
...
typedef struct {
	...
	uint8_t ns_para; /**< value range:0,1,2, the lower the noise, the lower the level, the default valude is recommended(推荐) */
...
typedef struct {
	...
	/* 增益 过高会导致失真、啸叫，爆音  过低会导致音量太小或听不见 */
	uint8_t mic_gain; /**< mic gain: value range:0x0 ~ 0x3f, suggest:0x2d */
	uint16_t spk_gain; /**< spk gain: value range:0x0 ~ 0x3f, suggest:0x2d */
	aud_dac_work_mode_t spk_mode; /**< audio spk mode: signal_ended/differen aud_dac_work_mode_t */
	aud_intf_voc_aec_cfg_t aec_cfg; 

	aud_intf_voc_mic_ctrl_t mic_en; /**< mic default status  aud_intf_voc_mic_ctrl_t */
	aud_intf_voc_spk_ctrl_t spk_en;	/**< spk default status aud_intf_voc_spk_ctrl_t */
	aud_intf_mic_type_t mic_type; /**< audio mic type: uac or microphone aud_intf_mic_type_t */
	aud_intf_spk_type_t spk_type; /**< audio speaker type: uac or speaker aud_intf_spk_type_t */
	aud_adc_mode_t mic_mode; /**< audio mic mode: signal_ended/differen aud_adc_mode_t */
	uint8_t frame_num; /**< the max number of frames in rx buffer to save rx data, the time of one frame is 20ms */
	/* 先缓存，再播放 fifo_frame_num 至少存了这么多帧才会开始播放 */
